...
const Grain = () => {
    const [grain, setGrain] = useState<any[]>([])
    ...
    useEffect(() => {
        axios.get("/data.json")
            .then((res) => {
                const filteredGrains = res.data.filter((item: any) => item.category === "Grain & Cereal");
                setGrain(filteredGrains);
                setLoading(false);
            })
            .catch(() => {
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 max-w-screen-2xl mx-auto gap-4">
            {grain.length > 0 ? (
                grain.map((item, index) => ( // Use `grain` here, not `Grain`
                ...
                <div>No Grain products found.</div>
            )}
        </div>
    );
};
